...
// UPDATED: MusicPlayer using consolidated hook
export function MusicPlayerWithConsolidatedState({
  isHost = false,
  roomId = '',
  ...
  const {
    ...
    hasAccessToken,
    refresh
  } = usePlaybackState(isHost, roomId, 5000, true);

  // Respond to external refresh triggers
  useEffect(() => {
    if (hasAccessToken && externalRefreshTrigger > 0) {
      console.log('External refresh trigger detected, updating playback data...');
      refresh();
    }
  }, [externalRefreshTrigger, hasAccessToken, refresh]);
      ...
      <div>
        ...
        {queue.length > 0 ? (
          <div className="bg-white rounded-lg shadow-sm max-h-64 overflow-y-auto">
            {queue.slice(0, 10).map((song: any, index: number) => (
              <div
                key={`${song.id}-${index}`}
...
export default function MusicPlayer({
  ...
  const fetchPlaybackData = async () => {
    await Promise.all([fetchCurrentSong(), fetchQueue()]);
  };
  ...
  // Auto-refresh playback data at specified interval
  useEffect(() => {
    if (!hasAccessToken) {
      return;
    }

    // Set up interval to refresh playback data
  ...
  }, [hasAccessToken, refreshInterval]);
